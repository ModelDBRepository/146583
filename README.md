...
- 3: GC information  
- 4: correlation  
  - blue: mean correlation of all pairs  
...
- sim: level of down sampling  
  - for the default input set, sim 40 -> 26 channels, sim 10 -> 442 channels  
- odor_names: file names of patterns from [http://gara.bio.uci.edu/](http://gara.bio.uci.edu/)  
- choose: odors chosen to be in the training set  

- non_lin: 0 for linear network, 1 for rectified nonlinear network (much slower)  
- conn: number of connection each GC makes (mean # connection for prob_conn = 1)  
...
- rm, rg: thresholds for rectifier, only works for non_lin == 1  
